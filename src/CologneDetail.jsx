// src/CologneDetail.jsx

import React, { useEffect, useState } from "react";
import { useParams, Link, useNavigate } from "react-router-dom";
import "./CologneDetail.css";

export default function CologneDetail() {
  const { id } = useParams();
  ...
  useEffect(() => {
    fetch(`http://localhost:8000/api/accounts/menscolognes/${id}/`)
      .then((res) => {
  ...
  const handleAddToCart = async () => {
    ...
    try {
      const response = await fetch("http://localhost:8000/api/accounts/cart/add/", {
        method: "POST",
        headers: {
  ...
  return (
    <div className="detail-container">
      <Link to="/mens" className="back-link">← Back to Men’s</Link>
